                ...
                return (
                  <div key={index} className="flex items-center pb-3">
                    <div className="relative w-full bg-gray-200 rounded-full overflow-hidden h-6">
                      <div
                        className={`absolute left-0 top-0 h-full ${votes > 0 ? 'bg-blue-500' : 'bg-gray-300'} text-black text-xs font-semibold flex items-center pl-2`}
                        style={{ width: `${votes > 0 ? percentage : 0}%` }}
                      >
            ...
            {options.map((option, index) => {
              ...
              return (
                <div key={index} className="flex flex-col mb-4">
                  <div className="text-lg font-semibold text-gray-700 mb-1">{option}</div>
                  <div className="relative w-full bg-gray-200 rounded-full overflow-hidden h-6">
                    <div
                      className={`absolute left-0 top-0 h-full ${votes > 0 ? 'bg-blue-500' : 'bg-gray-300'} text-black text-xs font-semibold flex items-center pl-2`}
                      style={{ width: `${votes > 0 ? percentage : 0}%` }}
            ...
            {options.map((_, index) => {
              ...
              return (
                <div key={index} className="border-t pt-2 mb-4">
                  <h3 className="font-bold text-gray-800 text-center mb-2">{`Option ${index + 1}`}</h3>
                  {voters.length > 0 ? (
                    voters.map((voter, i) => (
